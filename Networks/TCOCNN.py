...
class TCOCNNClass:
    ...
    def build_net(self, optim):
        ...
        model.add(layers.Conv2D(n_filters, (1, first_kernel_size), strides=(1, first_stride), padding='same',kernel_regularizer=regularizers.l2(self.l2_reg)))
        model.add(layers.BatchNormalization(momentum=momentumV))
        model.add(layers.ReLU())
        model.add(layers.Conv2D(n_filters, (1, first_kernel_size), strides=(1, 1), padding='same',kernel_regularizer=regularizers.l2(self.l2_reg)))
        model.add(layers.BatchNormalization(momentum=momentumV))
        model.add(layers.ReLU())

        # Additional Convolutional Blocks
        for i in range(1, num_convs):
            model.add(layers.Conv2D(n_filters * (i + 1), (1, 2), strides=(1, 2), padding='same',kernel_regularizer=regularizers.l2(self.l2_reg)))
            model.add(layers.BatchNormalization(momentum=momentumV))
            model.add(layers.ReLU())
            model.add(layers.Conv2D(n_filters * (i + 1), (1, 2), strides=(1, 1), padding='same',kernel_regularizer=regularizers.l2(self.l2_reg)))
            model.add(layers.BatchNormalization(momentum=momentumV))
            model.add(layers.ReLU())

        # Fully Connected Layers
        model.add(layers.Flatten())
        model.add(layers.Dense(width_fc, activation='relu',kernel_regularizer=regularizers.l2(self.l2_reg)))
